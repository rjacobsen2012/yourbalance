<script <?php if(isset($csp_script_nonce) && $csp_script_nonce): ?> nonce="<?php echo e($csp_script_nonce); ?>" <?php endif; ?>>
    Sfjs = (function() {
        "use strict";
        ...
        <?php if(isset($excluded_ajax_paths)): ?>
        if (window.fetch && window.fetch.polyfill === undefined) {
            ...
            window.fetch = function () {
                ...
                if (!url.match(new RegExp(<?php echo json_encode($excluded_ajax_paths, 15, 512) ?>))) {
                    var method = 'GET';
        ...
        if (window.XMLHttpRequest && XMLHttpRequest.prototype.addEventListener) {
            ...
            XMLHttpRequest.prototype.open = function(method, url, async, user, pass) {
                ...
                var path = url;
                if (url.substr(0, 1) === '/') {
                    if (0 === url.indexOf('<?php echo e(asset('/')); ?>')) {
                        path = url.substr(<?php echo e(strlen( asset('/'))); ?>);
                    }
                }
                else if (0 === url.indexOf('<?php echo e(url('/')); ?>')) {
                    path = url.substr(<?php echo e(strlen( url('/'))); ?>);
                }

                if (!path.match(new RegExp(<?php echo json_encode($excluded_ajax_paths, 15, 512) ?>))) {
                    var stackElement = {
                        error: false,
                        url: path,
                        method: method,
        ...
        return {
            ...
            loadToolbar: function(token, newToken) {
                ...
                this.load(
                    'sfwdt' + token,
                    '<?php echo e(route("telescope-toolbar.render", ["token" => "xxxxxx"])); ?>'.replace(/xxxxxx/, newToken),
                    function(xhr, el) {
                    ...
                    function(xhr) {
                        if (xhr.status !== 0) {
                            ...
                            var url = '<?php echo e(route("telescope")); ?>';
                            sfwdt.innerHTML = '\
...
<?php /**PATH /Users/richardjacobsen/development/rj-app/vendor/fruitcake/laravel-telescope-toolbar/src/../resources/views/base_js.blade.php ENDPATH**/ ?>